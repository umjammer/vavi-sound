...
class SmafExclusiveTest {
    ...
    void voice() {
        ...
        byte[] exclusive = YamahaExclusive.voice(0, 3, 0x41, 36, YamahaExclusive.VoiceType.FM, image);
    ...
    void voiceType() {
        ...
        assertEquals(0, YamahaExclusive.VoiceType.FM.ordinal());
        assertEquals(1, YamahaExclusive.VoiceType.PCM.ordinal());
        assertEquals(2, YamahaExclusive.VoiceType.AL.ordinal());
    }
    ...
    void wave() {
        ...
        byte[] exclusive = YamahaExclusive.wave(3, adpcm);
    ...
    void packIsTheSameAsYamahaMessage() throws Exception {
        byte[] exclusive = YamahaExclusive.voice(0, 0, 0x41, 0, YamahaExclusive.VoiceType.PCM, new byte[16]);
        ...
        assertArrayEquals(yamahaMessage.getMidiEvents(new MidiContext())[0].getMessage().getMessage(),
                StreamExclusive.pack(exclusive).getMessage());
    }
    ...
    void packRoundTrip() throws Exception {
        ...
        for (int length = 1; length < 64; length++) {
            ...
            capture.clear();
            YamahaExclusive.send(capture, exclusive);
    ...
    void packedSysexIsAVaviSysex() throws Exception {
        SysexMessage message = StreamExclusive.pack(YamahaExclusive.wave(0, new byte[] {0x01, 0x02}));

        assertEquals(0xf0, message.getStatus());
        assertEquals(VaviMidiDeviceProvider.MANUFACTURER_ID, message.getData()[0]);
        assertEquals(StreamExclusive.SYSEX_PACKED, message.getData()[1]);
    }

    /** a null receiver is not an error, the message it comes from is decoded either way */
    @Test
    void sendToNothing() {
        YamahaExclusive.send(null, YamahaExclusive.wave(0, new byte[0]));
    }
}
