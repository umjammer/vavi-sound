 ...
 */
public class MsFormatConversionProvider extends FormatConversionProvider {

    @Override
    public AudioFormat.Encoding[] getSourceEncodings() {
        return new AudioFormat.Encoding[] { MsEncoding.MS, AudioFormat.Encoding.PCM_SIGNED };
    }
    ...
    public AudioFormat.Encoding[] getTargetEncodings(AudioFormat sourceFormat) {
        if (sourceFormat.getEncoding().equals(AudioFormat.Encoding.PCM_SIGNED)) {
            return new AudioFormat.Encoding[] { MsEncoding.MS };
        } else if (sourceFormat.getEncoding() instanceof MsEncoding) {
    ...
    public AudioFormat[] getTargetFormats(AudioFormat.Encoding targetEncoding, AudioFormat sourceFormat) {
        if (sourceFormat.getEncoding().equals(AudioFormat.Encoding.PCM_SIGNED) &&
            targetEncoding instanceof MsEncoding) {
        ...
        } else if (sourceFormat.getEncoding() instanceof MsEncoding && targetEncoding.equals(AudioFormat.Encoding.PCM_SIGNED)) {
            return new AudioFormat[] {
        ...
        if (isConversionSupported(targetEncoding, sourceStream.getFormat())) {
            ...
            if (formats != null && formats.length > 0) {
                ...
                if (sourceFormat.equals(targetFormat)) {
                    return sourceStream;
                } else if (sourceFormat.getEncoding() instanceof MsEncoding && targetFormat.getEncoding().equals(AudioFormat.Encoding.PCM_SIGNED)) {
                    try {
                        int samplesPerBlock = (int) sourceFormat.getProperty("samplesPerBlock");
                        int nCoefs = (int) sourceFormat.getProperty("nCoefs");
                        int[][] iCoefs = (int[][]) sourceFormat.getProperty("iCoefs");
                        int blockSize = (int) sourceFormat.getProperty("blockSize");
                        return new Ms2PcmAudioInputStream(sourceStream, targetFormat, AudioSystem.NOT_SPECIFIED, samplesPerBlock, nCoefs, iCoefs, blockSize);
                    } catch (IOException e) {
