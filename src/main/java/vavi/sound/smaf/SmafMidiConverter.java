...
//Debug.println("Å°Å°Å°Å°Å°(" + i + ":" + j + ") ticks: " + midiContext.getCurrentTick() + "(" + midiContext.getTicksOf(smafMessage.getDuration()) + "," + smafMessage.getDuration() + "), " + smafMessage.getClass().getSimpleName());
...
// if (gateTime == 0) {
//  Debug.println(Level.WARNING, "ÅöÅöÅöÅöÅö(" + i + ":" + j + ") gateTime == 0: " + smafMessage);
// }
//}
...
Debug.println("unhandled message: " + smafMessage);
    ...
    /** Note Ç™ Control/Program ÇÊÇËêÊÇ…ì¸ÇÈÇ±Ç∆Ç™Ç†ÇÈ */
    @SuppressWarnings("unused")
    private void addSmafMessage(javax.sound.midi.Track midiTrack, MidiEvent midiEvent) {
//Debug.println("Åö: " + midiEvent.getMessage());
//Debug.println("Åö: " + (midiTrack.size() > 1 ? midiTrack.get(midiTrack.size() - 2).getMessage() : null));
...
Debug.println("ÅöÅöÅöÅöÅö : " + MidiUtil.paramString(midiEvent.getMessage()) + ", " + MidiUtil.paramString(removedMidiEvent.getMessage()));
        } else {
            midiTrack.add(midiEvent);
        }
    }
